...
namespace AhmadSanitary
{
    public partial class Purchases_Detail : Form
    {
        ...
        private void Purchases_Detail_Load(object sender, EventArgs e)
        {
            ...
            string command2 = "select s.p_id P_Code,p.p_name P_Name,s.White, s.Ivory, s.Blue, s.S_Green, s.Pink, s.Gray, s.Beige, s.Burgundy, s.Peacock, s.Mauve, s.Black, s.Brown, s.D_Blue, s.CP, s.T_Qty, s.pprice Price,CAST(ROUND((((s.t_qty) * (s.pprice)) - (s.discount)),2) as decimal(18,2)) Amount From purchase_stock_detail2 s, product2 p Where s.p_id=p.p_id AND s.ps_id ='" + invoice_no + "'";
