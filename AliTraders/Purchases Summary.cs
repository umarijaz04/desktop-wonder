...
{
    public partial class Purchases_Summary : Form
    {
        Form opener;
        Decimal amt;
        UF.Class1 test = new UF.Class1();

        public Purchases_Summary(Form from)
        {
            InitializeComponent();
            opener = from;
        }

        private void Purchases_Summary_Load(object sender, EventArgs e)
        {
            opener.Enabled = false;
        }

        private void Purchases_Summary_FormClosing(object sender, FormClosingEventArgs e)
        {
            opener.Enabled = true;
        }
        ...
        {
            amt = 0;
            test.bindingcode("select case when c.type='FIXED' then 'Customer' else 'Stakeholder' end as AC_Type, c.c_name AC_Name, isnull(sum(s.total),0) Amount from customer c, purchase_stock s where s.c_id=c.c_id AND (datetime Between '" + Convert.ToDateTime(dateTimePicker1.Text) + "' AND '" + Convert.ToDateTime(dateTimePicker2.Text).AddDays(1) + "') group by c.c_name,c.type UNION ALL select 'Manufacturer' AC_Type, m.name AC_Name, isnull(sum(s.total),0) Amount from manufacturer m, purchase_stock s where s.m_id=m.m_id AND (datetime Between '" + Convert.ToDateTime(dateTimePicker1.Text) + "' AND '" + Convert.ToDateTime(dateTimePicker2.Text).AddDays(1) + "') group by m.name", dataGridView, Login.con);
        ...
        private void printDocument1_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            int startY = 150;
            int Offset = 40;
            e.Graphics.DrawString("Purchases Summary", new Font("Calibri", 12, FontStyle.Underline), new SolidBrush(Color.Black), startX, startY + Offset);
            Offset = Offset + 30;
            string underLine = "--------------------------------------------------------------------------------";
