...
namespace AhmadSanitary
{
    ...
    {
        Form opener;
        Decimal balance, sdebit, scredit, prdebit, prcredit, srdebit, srcredit, tdebit, tcredit, pre_balance, pcredit, pdebit, amt;
        UF.Class1 test = new UF.Class1();

        private int i = 0;

        public Receivable(Form form)
        {
            InitializeComponent();
            opener = form;
        }
        ...
        private void Receivable_FormClosing(object sender, FormClosingEventArgs e)
        {
            opener.Enabled = true;
        }
        ...
        void prebalance()
        {
            ...
            if (panelmanu.Visible == false)
            {
                ...
                lbreccus.Text = (amt = (Decimal)cmd8.ExecuteScalar()).ToString();
                Login.con.Close();
            ...
            else if (panelmanu.Visible == true)
            {
                ...
                SqlCommand cmd8 = new SqlCommand("Select isnull(sum(balance),0) From prebalance", Login.con);
                lbrecmanu.Text = (amt = (Decimal)cmd8.ExecuteScalar()).ToString();
                Login.con.Close();
        ...
        private void printDocument1_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            int startY = 150;
            int Offset = 40;
            e.Graphics.DrawString("" + cmbaddcustomer.Text.ToUpper() + " Customer Receivable Balance", new Font("Calibri", 12, FontStyle.Underline), new SolidBrush(Color.Black), startX, startY + Offset);
            Offset = Offset + 30;
            string underLine = "--------------------------------------------------------------------------------";
        ...
        private void printDocument2_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            int startY = 150;
            int Offset = 40;
            e.Graphics.DrawString("Manufacturer Receivable Balance", new Font("Calibri", 12, FontStyle.Underline), new SolidBrush(Color.Black), startX, startY + Offset);
            Offset = Offset + 30;
            string underLine = "--------------------------------------------------------------------------------";
            e.Graphics.DrawString(underLine, new Font("Courier New", 10, FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
            Offset = Offset + 20;
