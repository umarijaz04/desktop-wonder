...
{
    public partial class Product_Wise_Purchases_Return : Form
    {
        Form opener;
        Int32 db_tiles;
        Decimal qty, tile;
        ...
        public Product_Wise_Purchases_Return(Form form)
        {
            InitializeComponent();
            opener = form;
        }

        private void Product_Wise_Purchases_Return_Load(object sender, EventArgs e)
        {
        ...
        private void Product_Wise_Purchases_Return_FormClosing(object sender, FormClosingEventArgs e)
        {
            opener.Enabled = true;
        }
        ...
        //{
        //    string command1 = "select s.pr_id Purchases_Return_No, ss.datetime DateTime,case when c.type='FIXED' then 'Customer' else 'Stakeholder' end as AC_Type,c.c_name AC_Name,s.p_id P_Code,s.tone Tone, s.quality Quality, s.size Size, s.qty Boxes, s.tiles Tiles, CAST(ROUND((p.meters*(s.qty + (s.tiles / p.pieces))),2) as decimal(18,2)) Meters, s.price Price, s.deduct Deduct, CAST(ROUND((((s.qty + (s.tiles / p.pieces)) * (s.price)) - (((s.qty + (s.tiles / p.pieces)) * (s.price) * (s.deduct))/100)),2) as decimal(18,2)) Amount from customer c, purchase_return_detail s, purchase_return ss, product p where c.c_id=ss.c_id AND p.p_id=s.p_id AND p.size=s.size AND s.pr_id=ss.pr_id AND (ss.datetime between '" + Convert.ToDateTime(dateTimePicker1.Text) + "' AND '" + Convert.ToDateTime(dateTimePicker2.Text).AddDays(1) + "') AND s.p_id like'" + cmbcode.Text + "%' AND s.size like'" + cmbsize.Text + "%' AND s.quality like'" + cmbquality.Text + "%' AND s.tone like'" + cmbtone.Text + "%' UNION ALL select s.pr_id Purchases_Return_No, ss.datetime DateTime,'Manufacturer' AC_Type,m.name AC_Name,s.p_id P_Code,s.tone Tone, s.quality Quality, s.size Size, s.qty Boxes, s.tiles Tiles, CAST(ROUND((p.meters*(s.qty + (s.tiles / p.pieces))),2) as decimal(18,2)) Meters, s.price Price, s.deduct Deduct, CAST(ROUND((((s.qty + (s.tiles / p.pieces)) * (s.price)) - (((s.qty + (s.tiles / p.pieces)) * (s.price) * (s.deduct))/100)),2) as decimal(18,2)) Amount from manufacturer m, purchase_return_detail s, purchase_return ss, product p where m.m_id=ss.m_id AND p.p_id=s.p_id AND p.size=s.size AND s.pr_id=ss.pr_id AND (ss.datetime between '" + Convert.ToDateTime(dateTimePicker1.Text) + "' AND '" + Convert.ToDateTime(dateTimePicker2.Text).AddDays(1) + "') AND s.p_id like'" + cmbcode.Text + "%' AND s.size like'" + cmbsize.Text + "%' AND s.quality like'" + cmbquality.Text + "%' AND s.tone like'" + cmbtone.Text + "%'";
        ...
        private void printDocument1_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            //int startY = 150;
            //int Offset = 40;
            //e.Graphics.DrawString("Product Wise Purchases Return", new Font("Calibri", 12, FontStyle.Underline), new SolidBrush(Color.Black), startX, startY + Offset);
            //Offset = Offset + 30;
            ...
            //Offset = Offset + 20;
            //e.Graphics.DrawString("Purchases R#", new Font("Calibri", 10, FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
        ...
        {
            string command1 = "select s.pr_id Purchases_Return_No, ss.datetime DateTime,case when c.type='FIXED' then 'Customer' else 'Stakeholder' end as AC_Type,c.c_name AC_Name, s.qty Quantity, s.price Price, s.deduct Deduct, CAST(ROUND(((s.qty * s.price) - (((s.qty * s.price)* s.deduct)/100)),2) as decimal(18,2)) Amount from customer c, purchase_return_detail2 s, purchase_return ss, product2 p where c.c_id=ss.c_id AND p.p_id=s.p_id AND s.pr_id=ss.pr_id AND (ss.datetime between '" + Convert.ToDateTime(dateTimePicker4.Text) + "' AND '" + Convert.ToDateTime(dateTimePicker3.Text).AddDays(1) + "') AND p.p_id='" + txtcode.Text + "' UNION ALL select s.pr_id Purchases_Return_No, ss.datetime DateTime,'Manufacturer' AC_Type,m.name AC_Name, s.qty Quantity, s.price Price, s.deduct Deduct, CAST(ROUND(((s.qty * s.price) - (((s.qty * s.price)* s.deduct)/100)),2) as decimal(18,2)) Amount from manufacturer m, purchase_return_detail2 s, purchase_return ss, product2 p where m.m_id=ss.m_id AND p.p_id=s.p_id AND s.pr_id=ss.pr_id AND (ss.datetime between '" + Convert.ToDateTime(dateTimePicker4.Text) + "' AND '" + Convert.ToDateTime(dateTimePicker3.Text).AddDays(1) + "') AND p.p_id='" + txtcode.Text + "'";
