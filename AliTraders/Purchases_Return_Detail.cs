...
{
    public partial class Purchases_Return_Detail : Form
    {
        Decimal invoice_no, c_id;
        UF.Class1 test = new UF.Class1();

        public Purchases_Return_Detail(decimal Invoice_No)
        {
            InitializeComponent();
        ...
        private void Purchases_Return_Detail_Load(object sender, EventArgs e)
        {
            Login.con.Open();
            SqlCommand cmd = new SqlCommand("select * from purchase_return where pr_id='" + invoice_no + "'", Login.con);
            ...
            if (dr.HasRows)
            {
                while (dr.Read())
                {
                    ...
                    labeldatetime.Text = Convert.ToString(dr[8]);
                    labeltotal.Text = Convert.ToString(dr[6]);
                    labelreceive.Text = Convert.ToString(dr[7]);
                    labelbalance.Text = Convert.ToString((Convert.ToDecimal(dr[6]) - Convert.ToDecimal(dr[7])));
                    ...
                    else if (Convert.ToString(dr[2]) != DBNull.Value.ToString())
                    {
                        c_id = Convert.ToDecimal(dr[2]);
                        dr.Close();
            ...
            //string command = "select s.p_id P_Code,s.tone Tone,s.quality Quality,s.size Size,s.qty Boxes,s.tiles Tiles, CAST(ROUND((p.meters*((s.tiles / p.pieces) + s.qty)),2)as decimal(18,2)) Meters, s.price Price, s.deduct Deduct, CAST(ROUND((((s.qty + (s.tiles / p.pieces)) * (s.price)) - (((s.qty + (s.tiles / p.pieces)) * (s.price) * (s.deduct))/100)),2) as decimal(18,2)) Amount From purchase_return_detail s, product p Where s.p_id=p.p_id AND s.size=p.size AND s.pr_id ='" + invoice_no + "'";
            //test.bindingcode(command, dataGridView, Login.con);
            string command2 = "select s.p_id P_Code,p.p_name P_Name,s.qty Quantity, s.price Price, s.deduct Deduct, CAST(ROUND(((s.qty * s.price) - (((s.qty * s.price)* s.deduct)/100)),2) as decimal(18,2)) Amount From purchase_return_detail2 s, product2 p Where s.p_id=p.p_id AND s.pr_id ='" + invoice_no + "'";
        ...
        private void printDocument1_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            int startY = 150;
            int Offset = 40;
            e.Graphics.DrawString("Purchase Return No:", new Font("Calibri", 12), new SolidBrush(Color.Black), startX, startY + Offset);
            e.Graphics.DrawString(labelinvoiveno.Text, new Font("Calibri", 12, FontStyle.Bold), new SolidBrush(Color.Black), startX + 145, startY + Offset);
            Offset = Offset + 30;
            ...
            e.Graphics.DrawString(((Convert.ToDecimal(labeltotal.Text)) - Convert.ToDecimal(labelreceive.Text)).ToString(), new Font("Calibri", 10), new SolidBrush(Color.Black), startX + 665, startY + Offset);
            Offset = Offset + 20;
            e.Graphics.DrawString(underLine, new Font("Courier New", 10, FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
            Offset = Offset + 20;
        ...
        private void printDocument2_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            //    e.Graphics.DrawString(underLine, new Font("Courier New", 10, FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
            //    Offset = Offset + 20;
            //    e.Graphics.DrawString("Pr#", new Font("Calibri", 10, FontStyle.Bold), new SolidBrush(Color.Black), startX, startY + Offset);
            //    e.Graphics.DrawString("P_Code", new Font("Calibri", 10, FontStyle.Bold), new SolidBrush(Color.Black), startX + 45, startY + Offset);
